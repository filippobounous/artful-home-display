...
export const generateTestData = (): DecorItem[] => {
  ...
  const categories = [
    {
      id: 'art',
      name: 'Art',
      subcategories: ['painting', 'sculpture', 'photography', 'print'],
    },
    {
      id: 'furniture',
      name: 'Furniture',
      subcategories: ['chair', 'table', 'sofa', 'cabinet', 'rug'],
    },
    {
      id: 'decorative',
      name: 'Decorative',
      subcategories: ['vase', 'basket', 'mirror', 'lighting'],
    },
  ...
  const conditions: ('mint' | 'excellent' | 'very good' | 'good')[] = [
    'mint',
    'excellent',
    'very good',
    'good',
  ...
  const generateItem = (id: number): DecorItem => {
    ...
    const valuationValue = Math.round(
      acquisitionValue * (0.8 + Math.random() * 0.4),
    );
    ...
    const baseName = getRandomElement(
      itemNames[category.id as keyof typeof itemNames] || ['Object'],
    );
    const title =
      Math.random() > 0.3
    ...
    return {
      ...
      valuationPerson: getRandomElement([
        ...
        'Margaret St. Claire',
        'European Art Consultants',
        'Fine Arts Appraisal Services',
      ]),
  ...
  const items = Array.from({ length: 150 }, (_, i) => generateItem(i + 1));

  const warningItems: DecorItem[] = Array.from({ length: 10 }, (_, i) => {
    const item = generateItem(151 + i);
    switch (i) {
      case 0:
        item.title = '';
        break;
